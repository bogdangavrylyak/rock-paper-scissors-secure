...
contract RockPaperScissors {
  ...
  function revealChoice(
    ...
    require(
      ...
      'Not all players set their choices and addresses'
    );

    bytes32 hashedChoice = HashLib.hashChoice(_player1choice, _player1salt);

    if (hashedChoice != game.player1HashedChoice) {
  ...
  function checkWin(Game memory game, uint _gameId) internal {
    if (game.player1Choice == game.player2Choice) {
      ...
      (game.player1Choice == Choice.Rock &&
        game.player2Choice == Choice.Scissors) ||
      (game.player1Choice == Choice.Paper &&
        game.player2Choice == Choice.Rock) ||
      (game.player1Choice == Choice.Scissors &&
        game.player2Choice == Choice.Paper)
    ) {
      emit GameFinished(
        _gameId,
        game.player1,
